...
from torch_geometric.utils import negative_sampling, to_undirected, structured_negative_sampling
...
class RSGNN(nn.Module):
    ...
    def init_models(self):
        ...
        self.encoder_to_decoder = nn.Linear(self.n_class, self.nhid)
    ...
    def recons_loss(self, z, edge_index, stepwise=True):
        ...
        rec_loss = (F.mse_loss(neg,torch.zeros_like(neg)-1, reduction='sum') + \
        ...
        for i in trange(iteration):
            for epoch in range(100):
                ...
                for i in range(2):
                    ...
                    embeddings_ = self.model_s.get_embeds(self.x, real_edge_index, None, mask_rate=0.5)
                    # loss_neg = self.recons_loss_2(embeddings_, real_edge_index, torch.zeros(single_mask.sum().item()).bool())

                    # embeddings_ = self.model_s.get_embeds(self.x, self.edge_index, self.edge_weight, mask_rate=0.5)
                    embeddings_ = self.encoder_to_decoder(embeddings_)
                    reconst = self.decoder.get_embeds(embeddings_, real_edge_index, None, mask_nodes=self.model_s.mask_nodes)
                    loss_mask = self.mask_feature_loss(self.x, reconst, loss_type='cos')
    ...
    def add_nodes(self, train_mask, n=200):
    ...
    def get_real_edge_mask(self, edge_index, knn_edge_index):
        mask = torch.zeros_like(knn_edge_index[0]).bool()
        for src, dst in edge_index.T:
            mask1 = torch.logical_and(knn_edge_index[0] == src, knn_edge_index[1] == dst)
            mask2 = torch.logical_and(knn_edge_index[1] == src, knn_edge_index[0] == dst)
            mask += mask1 + mask2
