...
class CoG(nn.Module):
    ...
    def init_models(self):
        ...
        self.decoder = GCN(16, self.nhid, self.nfeat, self.nhid, mask=True)
        self.model_s = GCN(self.nfeat, 16, 16, self.n_class, mask=True)
        self.encoder_to_decoder = nn.Linear(16, self.nhid, bias=True)
    ...
    def fit(self, x, adj, labels, idx_train, idx_val=None, idx_test=None, epochs=200, iteration=25):
        ...
        for epoch in range(1000):
            for i in range(1):
                ...
                # real_edge_mask = self.get_real_edge_mask(real_edge_index, fake_edge_index)
                # self.poten_edge_index = torch.cat([real_edge_index, fake_edge_index[:, ~real_edge_mask]], -1)
            ...
            if epoch % 20 == 0:
                print(epoch, accs, train_mask.shape[0], self.edge_weight.shape[0], real_edge_index.shape[1], loss_lp.item(), best_acc, best_test_acc)
