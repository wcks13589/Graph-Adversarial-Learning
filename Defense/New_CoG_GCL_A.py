...
class CoG(nn.Module):
    ...
    def fit(self, x, adj, labels, idx_train, idx_val=None, idx_test=None, epochs=200, iteration=20):
        ...
        self.x = x
        # train_mask_with_fake = torch.cat([train_mask, torch.arange(len(idx_train))+self.n_real])
        # training_labels_with_fake = torch.cat([training_labels, labels[idx_train]])

        # 用來測試不加nodes
        self.x_new = x
        train_mask_with_fake = train_mask
        training_labels_with_fake = training_labels
        ...
        for i in trange(iteration):
            ...
            for epoch in range(epochs):
                ...
                # edge_mask = torch.logical_and(edge_mask, training_labels_with_fake[fake_edge_index[0]] == training_labels_with_fake[fake_edge_index[1]])
                ...
                self.edge_weight = adj[tuple(real_edge_index)] # torch.cat([adj[tuple(real_edge_index)], fake_edge_weight])
            ...
            add_nodes_f, add_nodes_s, pseudo_labels_f, pseudo_labels_s = self.add_nodes(x, self.x_new, self.edge_index, train_mask)
    ...
    def add_nodes(self, x, x_new, edge_index, train_mask, n=100):
