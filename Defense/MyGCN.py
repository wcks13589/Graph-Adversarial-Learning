...
class GCN(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4,
        ...
        self.gc3 = GraphConvolution(nclass, nhid, with_bias=with_bias)
        self.gc4 = GraphConvolution(nhid, nclass, with_bias=with_bias)
        self.gc5 = GraphConvolution(nfeat, nhid, with_bias=with_bias)
        self.gc6 = GraphConvolution(nhid, nclass, with_bias=with_bias)

    def forward(self, x_feat, adj):
        if self.with_relu:
            x = F.relu(self.gc1(x_feat, adj))
        else:
            x = self.gc1(x_feat, adj)

        x = F.dropout(x, self.dropout, training=self.training)
        x = self.gc2(x, adj)
        
        if self.with_relu:
            z = F.relu(self.gc5(x_feat, adj))
        else:
            z = self.gc5(x_feat, adj)

        z = F.dropout(z, self.dropout, training=self.training)
        z = self.gc6(z, adj)

        sim_matrix = _similarity(z, z)
        sim_matrix = sim_matrix * self.adj
        x = self.gc3(x, sim_matrix)
