...
class CoG(nn.Module):
    ...
    def fit(self, x, adj, labels, idx_train, idx_val=None, idx_test=None, epochs=200):
        ...
        best_acc = 0
        print(fake_x.shape, self.k)
        for i in trange(self.iteration):
            for epoch in range(epochs):
                optimizer.zero_grad()
                self.graph_learner.train()
                loss_sim = self.graph_learner.calc_loss(self.x)
                ...
                # loss_sim += self.recons_loss(embeddings, real_edge_index, stepwise=False)
        ...
        for i, xi in enumerate(x_tilde_list):
            for j, xj in enumerate(x_tilde_list):
                if i < j:
                    ...
                    r_loss = F.mse_loss(X_recon, X_input)
                    zi, zj = torch.split(z, xi.size(0))
                    z_loss = F.mse_loss(zi, zj)
                    c_loss = contrastive_loss(zi, zj)
