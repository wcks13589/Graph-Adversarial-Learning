...
class CoG(nn.Module):
    ...
    def init_models(self):
        ...
        self.decoder_ = GCN(self.nhid, self.nhid, self.nfeat, self.nhid, mask=True)
        self.encoder_to_decoder_ = nn.Linear(self.nhid, self.nhid)
    ...
    def fit(self, x, adj, labels, idx_train, idx_val=None, idx_test=None, epochs=200):
        ...
        optimizer = torch.optim.Adam(list(self.model_s.parameters())+
                                     ...
                                     list(self.encoder_to_decoder_.parameters()),
        ...
        for i in trange(self.iteration):
            for epoch in range(epochs):
                ...
                # self.encoder_to_decoder_.train()
                # self.decoder_.train()
                # z1 = self.encoder_to_decoder_(embeddings)
                # reconst = self.decoder_.get_embeds(z1, real_edge_index, None, mask_nodes=mask_nodes)
                ...
                loss_sim += self.recons_loss(embeddings, real_edge_index, stepwise=False) * 0.0001
