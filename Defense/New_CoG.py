...
class CoG(nn.Module):
    ...
    def fit(self, x, adj, labels, idx_train, idx_val=None, idx_test=None, epochs=200, iteration=20, fake_ratio=1):
        ...
        self.x = x
        train_mask_with_fake = torch.cat([train_mask, torch.arange(len(idx_train)*fake_ratio)+self.n_real])
        training_labels_with_fake = torch.cat([training_labels, labels[idx_train].repeat(fake_ratio)])
        ...
        for i in trange(iteration):
            ...
            self.x_new = torch.cat([x, x_fake.repeat(fake_ratio, 1)])
            ...
            for epoch in range(epochs):
                ...
                train_label_mask = torch.logical_and(torch.isin(fake_edge_index[0], train_mask_with_fake.cuda()), torch.isin(fake_edge_index[1], train_mask_with_fake.cuda()))
                label_mask = torch.logical_and(edge_mask, training_labels_with_fake[fake_edge_index[0]] == training_labels_with_fake[fake_edge_index[1]])
                edge_mask = torch.logical_and(label_mask, train_label_mask)

                sim_mask = torch.logical_and(sim_mask, ~train_label_mask)
                edge_mask = edge_mask + sim_mask
                ...
                if epoch % 20 == 0:
                    ...
                    # print((training_labels_with_fake[fake_edge_index[0]] == training_labels_with_fake[fake_edge_index[0]]).sum().item()/fake_edge_index.shape[1])
...
class Generator(nn.Module):
    def __init__(self, in_dim, out_dim, nclass):
        super().__init__()
        self.label_emb = nn.Embedding(nclass, 16)

        self.model = nn.Sequential(
            nn.Linear(in_dim + 16, 512),
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.Linear(256, 512),
...
class MLP_learner(nn.Module):
    ...
    def link_pred(self, pos_edges, neg_edges):
        embeddings = self.internal_forward(features)
        pos0 = embeddings[pos_edges[0]]
        pos1 = embeddings[pos_edges[1]]
