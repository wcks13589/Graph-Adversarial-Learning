...
class CoG(nn.Module):
    ...
    def init_models(self):
        ...
        self.encoder = GCN(self.nfeat, self.nhid, 64)
    ...
    def delete_edges(self, real_edge_index, embeddings):
        ...
        return real_edge_index[:, edge_mask], edge_weight[edge_mask], sim_matrix, edge_mask
    ...
    def contrast(self, real_edge_index, real_edge_weight, fake_edge_index, fake_edge_weight):
        ...
        z1 = self.model_s.project(self.x, fake_edge_index, fake_edge_weight)[:self.n_real]
        z2 = self.model_s.project(self.x, real_edge_index, real_edge_weight)[:self.n_real]
    ...
    def fit(self, x, adj, labels, idx_train, idx_val=None, idx_test=None, epochs=200, iteration=20, fake_rate=1):
        ...
        self.x = torch.cat([x, x])
        
        # x_fake, labels_fake = self.create_fake_nodes(x, training_labels, add_nodes_s)
        # self.x = torch.cat([self.x, x_fake.repeat(fake_rate, 1)])
        # training_labels = torch.cat([training_labels, training_labels])
        # train_mask = torch.cat([train_mask, train_mask+self.n_real])

        best_acc = 0
        embeds = []

        fake_edge_index = []
        for node in range(self.n_real):
            src = node
            dst = node+self.n_real
            fake_edge_index.append([src, dst])
            fake_edge_index.append([dst, src])
        fake_edge_index = torch.LongTensor(fake_edge_index).T.to(self.device)
        ...
        for i in trange(iteration):
            ...
            for epoch in range(epochs):
                optimizer.zero_grad()
                embeddings = self.graph_learner.internal_forward(self.x[:self.n_real],)
                loss_lp = self.recons_loss(embeddings, real_edge_index)
                
                new_edge_index, new_edge_weight, sim_matrix, edge_mask = self.delete_edges(real_edge_index, embeddings)
                
                # z = self.encoder.get_embeds(self.x[:self.n_real], new_edge_index)
                # # fake_edge_index, fake_edge_weight = knn_fast(z, self.k, 1000, self.device)
                # loss_mask = self.recons_loss(z, real_edge_index[:, ~edge_mask], stepwise=False)
                # new_edge_index, new_edge_weight, edge_mask = self.delete_edges(real_edge_index, z)

                temp_edge_index = real_edge_index[:, ~edge_mask]
                temp_edge_index[1] = temp_edge_index[1]+self.n_real
                # temp_edge_index = torch.cat([temp_edge_index, torch.stack([temp_edge_index[1], temp_edge_index[0]])], -1)

                # new_edge_index, new_edge_weight, edge_mask_2 = self.delete_edges(real_edge_index, z)

                # edge_mask = edge_mask_1 + edge_mask_2

                # new_edge_index, new_edge_weight = real_edge_index[:, edge_mask], 
                ...
                self.edge_index = torch.cat([new_edge_index, fake_edge_index, temp_edge_index], -1)
                self.edge_weight = None # torch.cat([new_edge_weight, torch.ones_like(fake_edge_index[0]), temp_edge_weight])

                uncertain_edge_index = real_edge_index[:, ~edge_mask]
                uncertain_edge_weight = sim_matrix[tuple(uncertain_edge_index)].relu()
                # loss_con = self.contrast(new_edge_index, new_edge_weight.relu(), uncertain_edge_index, uncertain_edge_weight)
                # z = self.gcl(self.x, new_edge_index, new_edge_weight)

                # print(new_edge_index.shape, fake_edge_index.shape, temp_edge_index.shape)
                ...
                loss = loss_lp + loss_s# + loss_mask * 0.0001
                ...
                if epoch % 20 == 0:
                    ...
                    # print(accs, train_mask.shape[0], real_edge_index.shape[1], self.edge_index.shape[1], best_acc, best_test_acc)

            # fake_adj = _similarity(embeddings[:self.n_real])
            # plt.figure()
            # new_mask_clean = fake_adj[tuple(real_edge_index[:, real_mask])] > -1
            # new_mask_attack = fake_adj[tuple(real_edge_index[:, ~real_mask])] > -1
            # sns.histplot(data=fake_adj[tuple(real_edge_index[:, real_mask])][new_mask_clean].detach().cpu(), bins=30, color='skyblue', stat='count')
            # sns.histplot(data=fake_adj[tuple(real_edge_index[:, ~real_mask])][new_mask_attack].detach().cpu(), bins=30, color='red', stat='count', alpha=0.6)
            ...
            # plt.savefig(f'./image/tfstplt_{i}.jpg')
        ...
        self.restore_all(best_model_s_wts, best_model_g_wts, best_model_e_wts, real_edge_index, real_edge_weight, fake_edge_index, training_labels, train_mask)

    def restore_all(self, model_s_wts, model_g_wts, model_e_wts, real_edge_index, real_edge_weight, fake_edge_index, training_labels, train_mask):
