...
from utils import knn_fast, add_edges
    ...
    def fit(self, x, adj, labels, idx_train, idx_val=None, idx_test=None, epochs=200, iteration=20):
        ...
        for i in trange(iteration):
            for epoch in range(epochs):
                ...
                fake_edge_index, fake_edge_weight = self.add_edges()
        ...
        self.restore_all(best_model_s_wts, best_model_g_wts, real_edge_index, real_edge_weight)

        torch.save(torch.cat(edge_index_, 0), 'fake_edge_index_dot')
        torch.save(torch.cat(edge_weight_, 0), 'fake_edge_weight_dot')

    def restore_all(self, model_s_wts, model_g_wts, real_edge_index, real_edge_weight):
