...
class CoG(nn.Module):
    ...
    def init_models(self):
        ...
        self.encoder = GCN(self.nfeat//8, self.nhid, 64)
    ...
    def contrast(self, real_edge_index, real_edge_weight, fake_edge_index, fake_edge_weight):
        ...
        z1 = self.gcl(self.x, fake_edge_index, fake_edge_weight)[:self.n_real]
        z2 = self.gcl(self.x[:self.n_real], real_edge_index, real_edge_weight)
    ...
    def fit(self, x, adj, labels, idx_train, idx_val=None, idx_test=None, epochs=200, iteration=20, fake_rate=1):
        ...
        add_nodes_s = torch.arange(self.n_real)
        ...
        # training_labels = torch.cat([training_labels, labels_fake.repeat(fake_rate)])
        # train_mask = torch.cat([train_mask, torch.arange(len(add_nodes_s)*fake_rate).to(self.device)+self.n_real+n])
        ...
        for i in trange(iteration):
            for epoch in range(epochs):
                ...
                z = self.encoder.get_embeds(embeddings, new_edge_index)
        ...
        self.restore_all(best_model_s_wts, best_model_g_wts, best_model_e_wts, real_edge_index, real_edge_weight, training_labels, train_mask)

    def restore_all(self, model_s_wts, model_g_wts, model_e_wts, real_edge_index, real_edge_weight, training_labels, train_mask):
