    ...
    def fit(self, x, adj, labels, idx_train, idx_val=None, idx_test=None, epochs=200, iteration=40):
        ...
        for i in trange(iteration):
            for epoch in range(epochs):
                ...
                embeddings = self.encoder.get_embeds(x, real_edge_index, real_edge_weight, mask_rate=0.5)
                self.decoder.mask_nodes = self.encoder.mask_nodes
                reconst = self.decoder.get_embeds(embeddings, real_edge_index, real_edge_weight)
                loss_mask = self.mask_feature_loss(x, reconst, self.decoder.mask_nodes)

                embeddings = self.encoder.get_embeds(x, real_edge_index, real_edge_weight)
                loss_lp = self.recons_loss(embeddings, real_edge_index)
            ...
            try:
                _, add_edges = edge_weight[unlabel_mask].topk(100)
        ...
        self.restore_all(x, best_model_s_wts, best_model_e_wts, real_edge_index, real_edge_weight, training_labels, train_mask)

    def restore_all(self,x,  model_s_wts, model_e_wts, real_edge_index, real_edge_weight, training_labels, train_mask):
...
class GCN(nn.Module):
    ...
    def get_embeds(self, x, edge_index, edge_weight=None, mask_rate=None):
        if mask_rate!=None or self.mask_nodes!=None:
            ...
            out_x[self.mask_nodes] = torch.Tensor([0.0])
