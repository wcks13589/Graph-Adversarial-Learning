...
class CoG(nn.Module):
    ...
    def init_models(self):
        ...
        self.encoder_1 = GCN(self.nfeat, self.nhid, self.nfeat//8)
        # self.encoder_2 = GCN(self.nfeat, self.nhid, self.n_class)
        self.decoder = GCN(self.nhid, self.nhid, self.nfeat, mask=True)
        self.model_s = GCN(self.nfeat, self.nhid, self.n_class, mask=True)
        self.encoder_to_decoder = nn.Linear(self.n_class, self.nhid, bias=True)
    ...
    def create_fake_nodes(self, num_fakes, x, labels, idx_train):
        if type(num_fakes) == int:
            ...
            assert self.n_class == len(num_fakes), "the length of 'num_fakes' must equal to nclass"
    ...
    def fit(self, x, adj, labels, idx_train, idx_val=None, idx_test=None, epochs=200, iteration=40):
        ...
        fake_x, fake_labels, fake_edge_index_0 = self.create_fake_nodes([0, 1000], x, labels, idx_train)
        print(x.shape, fake_edge_index_0.max())
        self.x = torch.cat([x, fake_x]) # torch.cat([x, x[idx_train]])
        train_mask = torch.cat([train_mask, torch.arange(len(fake_labels)).to(self.device)+self.n_real])
        training_labels = torch.cat([training_labels, fake_labels]) # torch.cat([training_labels, labels[idx_train]])
        ...
        for i in trange(iteration):
            for epoch in range(epochs):
                ...
                embeddings = self.model_s.get_embeds(x, real_edge_index, None, mask_rate=0.5, mask=True)
                embeddings = self.encoder_to_decoder(embeddings)
                self.decoder.mask_nodes = self.model_s.mask_nodes
                reconst = self.decoder.get_embeds(embeddings, real_edge_index, None, mask=True)
        ...
        #     plt.scatter(z[:self.n_real][labels.cpu() == i, 0], z[:self.n_real][labels.cpu() == i, 1], s=20, color=colors[i])
        #     plt.scatter(z[self.n_real:][fake_labels.cpu() == i, 0], z[self.n_real:][fake_labels.cpu() == i, 1], s=100, color=colors[i])
        # plt.savefig(f'./image/tsne.png')
        # torch.save(train_acc, './image/train_acc_mask_update')

        self.restore_all(x, best_model_s_wts, best_model_g_wts, real_edge_index, real_edge_weight, fake_edge_index_0, training_labels, train_mask, idx_train)

    def restore_all(self, x, model_s_wts, model_g_wts, real_edge_index, real_edge_weight, fake_edge_index_0, training_labels, train_mask, idx_train):
...
class GCN(nn.Module):
    ...
    def get_embeds(self, x, edge_index, edge_weight=None, mask_rate=None, mask=False):
