...
class CoG(nn.Module):
    ...
    def init_models(self):
        ...
        self.classifier = MLP(self.nhid, self.nhid, self.n_class)
    ...
    def fit(self, x, adj, labels, idx_train, idx_val=None, idx_test=None, epochs=200, iteration=20):
        ...
        for i in trange(iteration):
            ...
            for epoch in range(epochs):
                ...
                z = self.graph_learner.internal_forward(self.x)
                loss_lp = self.recons_loss(z, real_edge_index)
                new_edge_index, new_edge_weight, edge_mask = self.delete_edges(real_edge_index, z)

                z1 = self.encoder_to_decoder(self.encoder.get_embeds(self.x, new_edge_index))
                z2 = self.encoder_to_decoder(self.encoder.get_embeds(self.x, real_edge_index))
                loss_con = self.calc_loss(z1, z2, sym=True)

                # embeddings = self.encoder.get_embeds(x, self.edge_index, None, mask_rate=0.5, mask=True)
                # embeddings = self.encoder_to_decoder(embeddings)
                # self.decoder.mask_nodes = self.encoder.mask_nodes
                # reconst = self.decoder.get_embeds(embeddings, real_edge_index, real_edge_weight, mask=True)
                # loss_mask = self.mask_feature_loss(x, reconst, self.decoder.mask_nodes)

                self.edge_index, self.edge_weight = new_edge_index, None
                embeddings = self.encoder.get_embeds(self.x, self.edge_index, self.edge_weight)
                
                logit = self.classifier(embeddings)
                loss_s = F.nll_loss(logit[train_mask], training_labels[train_mask])

                loss = loss_s + loss_lp + loss_con
                loss.backward()
                optimizer.step()
                ...
                if epoch % 20 == 0:
                    ...
                    if accs[1] > best_acc:
                        ...
                        best_model_s_wts = copy.deepcopy(self.classifier.state_dict())
    ...
    def add_nodes(self, train_mask, n=100):
        ...
        unlabel_logit_s, unlabel_pseudo_s = logits[unlabel_nodes].max(-1)
