...
class CoG(nn.Module):
    ...
    def fit(self, x, adj, labels, idx_train, idx_val=None, idx_test=None, epochs=200):
        ...
        for i in trange(self.iteration):
            for epoch in range(200):
                ...
                # loss_c = F.nll_loss(logit[train_mask], training_labels[train_mask])
                # loss_sim += self.recons_loss(embeddings[:self.n_real], real_edge_index, stepwise=False)
                ...
                for j in range(1):
                    ...
                    # z1 = self.model_s.get_embeds(self.x, real_edge_index, None, mask_nodes=mask_nodes, mask_embedding=True)
    ...
    def add_nodes(self, train_mask, embeddings, real_edge_index, labels):
        ...
        hop_nodes = k_hop_subgraph(new_nodes_s, 2, self.edge_index)[0]
