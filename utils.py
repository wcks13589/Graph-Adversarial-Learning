...
def add_edges(fake_edge_index, fake_edge_weight, training_labels, train_mask, n_real, isolate_nodes=None, threshold=0.7, mode='mix'):
    ...
    if mode == 'mix':
        ...
        edge_mask = torch.logical_and(label_mask , sim_mask)
        ...
        train_label_mask = torch.logical_and(torch.isin(fake_edge_index[0], train_mask), torch.isin(fake_edge_index[1], train_mask))
        edge_mask = torch.logical_and(node_mask, training_labels[fake_edge_index[0]] == training_labels[fake_edge_index[1]])
